...
checkinhelp={
  ...
  init:function(){
	...
	var arrDropDivs = Array(
				//document.getElementById('site-navigation'),
				//document.getElementById('results'),
				document.getElementById('topnote'));
	checkinhelp.hideUnhide(arrDropDivs,0,Array());
	if (getUrlVar("type") != "New") {
		checkinhelp.toggleForm(0);
		var objIframe = document.getElementById('regdata');
		if (objIframe) {
			objIframe.src = 'loadregdata.php';
			objIframe.onload = function() {initSetup();};
		}
	}
  },

  toggleForm:function(intOnOff){
	var fieldsets=document.getElementsByTagName('fieldset');
	checkinhelp.hideUnhide(fieldsets, intOnOff, Array('aboutYou'));
	var items=document.getElementById('aboutYou').children;
	var arrDropItems = Array();
	for (var i=items.length; i > 0; --i) {
		if (items[i-1].getAttribute('id') == 'email') break;
		arrDropItems.push( items[i-1]);
	}
	arrDropItems.push(document.getElementById('Total'),document.getElementById('footnote'));
	checkinhelp.hideUnhide(arrDropItems, intOnOff, Array());

  },
	
  hideUnhide:function(lstElems,action,exclude){
    for(var i=0;i<lstElems.length;i++){
	if(exclude.contains(lstElems[i].getAttribute('id'))){continue;}
	if (action == 1) { DOMhelp.cssjs('remove', lstElems[i], checkinhelp.hidingClass); }
	else if (action == 0) { DOMhelp.cssjs('add', lstElems[i], checkinhelp.hidingClass); }
    }
  },
   
};

DOMhelp.addEvent(window,'load',checkinhelp.init,false);
